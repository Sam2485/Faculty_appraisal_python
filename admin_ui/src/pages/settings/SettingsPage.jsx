...
export default function SettingsPage() {
  ...
  const saveSection = async (section, keys) => {
    setSavingSection(section); setSaveErr(null);
  ...
  const handleSave = () => saveSection('urls', ['FRONTEND_URL', 'APP_URL', 'ALLOW_MOCK_USER']);
      ...
      {!loading && !error && tab === 'system' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
          ...
          <Card title="System Configuration" delay={0}>
            {[
              { l: 'Frontend URL', k: 'FRONTEND_URL', p: 'https://…', type: 'text' },
              { l: 'Backend URL',  k: 'APP_URL',      p: 'https://…', type: 'text' },
            ].map(f => (
              <div key={f.k} style={{ marginBottom: 14 }}>
                <label style={lbl}>{f.l}</label>
                <input className="ifield" type={f.type} value={form[f.k]} placeholder={f.p}
            ...
            <button className="act-btn" style={pBtn} onClick={handleSave}
